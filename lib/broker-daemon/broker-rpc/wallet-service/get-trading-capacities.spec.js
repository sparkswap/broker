...
describe('get-trading-capacities', () => {
    describe('getTradingCapacities', () => {
        ...
        it('throws an error if the base engine does not exist for symbol', () => {
            ...
            return expect(getTradingCapacities({ params, logger, engines, orderbooks, blockOrderWorker }, GetTradingCapacitiesResponse)).to.eventually.be.rejectedWith(`No engine available for BTC`);
        });
        it('throws an error if the counter engine does not exist for symbol', () => {
            engines = new Map([['BTC', baseEngineStub]]);
            return expect(getTradingCapacities({ params, logger, engines, orderbooks, blockOrderWorker }, GetTradingCapacitiesResponse)).to.be.rejectedWith(`No engine available for LTC`);
        });
...
//# sourceMappingURL=get-trading-capacities.spec.js.map