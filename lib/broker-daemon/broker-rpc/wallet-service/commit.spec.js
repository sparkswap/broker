...
describe('commit', () => {
    ...
    it('balance under minimum amount throws an error for an incorrect balance', () => {
        ...
        return expect(commit({ params, relayer, logger, engines, orderbooks }, { EmptyResponse })).to.be.rejectedWith(PublicError, 'Minimum balance of');
    });
    it('balance over allowed maximum value throws an error for an incorrect balance', () => {
        ...
        return expect(commit({ params, relayer, logger, engines, orderbooks }, { EmptyResponse })).to.be.rejectedWith(PublicError, 'Maximum balance');
    });
    ...
    it('throws an error if creating a channel fails', () => {
        ...
        expect(commit({ params, relayer, logger, engines, orderbooks }, { EmptyResponse })).to.be.rejectedWith(PublicError, 'Funding error');
    });
    ...
    describe('invalid engine types', () => {
        ...
        it('throws an error if inverse engine is not found', () => {
            ...
            return expect(commit({ params, relayer, logger, engines: badEngines, orderbooks }, { EmptyResponse })).to.be.rejectedWith(PublicError, 'No engine is configured for symbol');
        });
    });
    describe('checking channel balances', () => {
        it('throws an error if there are already open inbound and outbound channels with the desired amount', () => {
            ...
            return expect(commit({ params, relayer, logger, engines, orderbooks }, { EmptyResponse })).to.be.rejectedWith(PublicError, `You already have a channel open with ${params.balance} or greater.`);
        });
        it('throws an error if the outbound channel does not have the desired amount', () => {
            ...
            getMaxInboundChannelStub.resolves({ maxBalance: '300' });
            return expect(commit({ params, relayer, logger, engines, orderbooks }, { EmptyResponse })).to.be.rejectedWith(PublicError, 'You have another outbound channel open with a balance lower than desired, release that channel and try again.');
        });
        it('throws an error if the inbound channel does not have the desired amount', () => {
            ...
            return expect(commit({ params, relayer, logger, engines, orderbooks }, { EmptyResponse })).to.be.rejectedWith(PublicError, 'You have another inbound channel open with a balance lower than desired, release that channel and try again.');
        });
    });
});
