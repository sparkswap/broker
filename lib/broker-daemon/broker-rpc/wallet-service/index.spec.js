...
describe('WalletService', () => {
    ...
    describe('grpc implementations', () => {
        it('creates a unary method for newAddress', () => {
            ...
            expect(unaryMethodStub).to.have.been.calledWith(newDepositAddress, expectedMessageId, { logger, engines, auth }, { NewDepositAddressResponse: responseStub });
        });
        it('creates a unary method for getBalances', () => {
            const expectedMessageId = '[WalletService:getBalances]';
            expect(unaryMethodStub).to.have.been.calledWith(balanceSpy, expectedMessageId, { logger, engines, auth }, { GetBalancesResponse: responseStub });
        });
        it('creates a unary method for commit', () => {
            const expectedMessageId = '[WalletService:commit]';
            expect(unaryMethodStub).to.have.been.calledWith(commitSpy, expectedMessageId, { logger, engines, relayer, orderbooks, auth }, { EmptyResponse: responseStub });
        });
        it('creates a unary method for getPaymentChannelNetworkAddress', () => {
            ...
            expect(unaryMethodStub).to.have.been.calledWith(getPaymentChannelNetworkAddress, expectedMessageId, { logger, engines, auth }, { GetPaymentChannelNetworkAddressResponse: responseStub });
        });
        it('creates a unary method for getTradingCapacities', () => {
            const expectedMessageId = '[WalletService:getTradingCapacities]';
            expect(unaryMethodStub).to.have.been.calledWith(getTradingCapacities, expectedMessageId, { logger, engines, orderbooks, auth, blockOrderWorker }, { GetTradingCapacitiesResponse: responseStub });
        });
        it('creates a unary method for releaseChannels', () => {
            const expectedMessageId = '[WalletService:releaseChannels]';
            expect(unaryMethodStub).to.have.been.calledWith(releaseChannels, expectedMessageId, { logger, engines, orderbooks, auth }, { ReleaseChannelsResponse: responseStub });
        });
        it('creates a unary method for withdrawFunds', () => {
            const expectedMessageId = '[WalletService:withdrawFunds]';
            expect(unaryMethodStub).to.have.been.calledWith(withdrawFunds, expectedMessageId, { logger, engines, auth }, { WithdrawFundsResponse: responseStub });
        });
        it('creates a unary method for createWallet', () => {
            const expectedMessageId = '[WalletService:createWallet]';
            expect(unaryMethodStub).to.have.been.calledWith(createWallet, expectedMessageId, { logger, engines, auth }, { CreateWalletResponse: responseStub });
        });
        it('creates a unary method for unlockWallet', () => {
            const expectedMessageId = '[WalletService:unlockWallet]';
            expect(unaryMethodStub).to.have.been.calledWith(unlockWallet, expectedMessageId, { logger, engines, auth }, { EmptyResponse: responseStub });
        });
    });
});
