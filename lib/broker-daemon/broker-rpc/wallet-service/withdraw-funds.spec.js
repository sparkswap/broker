...
describe('withdrawFunds', () => {
    ...
    describe('withdrawFunds', () => {
        ...
        it('makes a request to the engine to withdrawFunds to the address specified', () => {
            expect(btcEngine.withdrawFunds).to.have.been.calledWith(params.address, Big(params.amount).times(100000000).toString());
        });
    ...
    describe('invalid engine types', () => {
        it('throws an error if engine is not found', () => {
            ...
            return expect(withdrawFunds({ params, relayer, logger, engines: badEngines }, { WithdrawFundsResponse })).to.be.rejectedWith(PublicError, `No engine available for ${params.symbol}`);
        });
    });
    describe('balance under minimum amount', () => {
        it('throws a PublicError on withdraw failure', () => {
            ...
            return expect(withdrawFunds({ params, relayer, logger, engines }, { WithdrawFundsResponse })).to.be.rejectedWith(PublicError, 'Insufficient Funds');
        });
    });
    describe('invalid currency multiplier', () => {
        ...
        it('throws an error currency multiplier does not exist', () => {
            return expect(withdrawFunds({ params, relayer, logger, engines }, { WithdrawFundsResponse })).to.be.rejectedWith(PublicError, `Invalid configuration: missing quantumsPerCommon for ${params.symbol}`);
        });
    });
});
//# sourceMappingURL=withdraw-funds.spec.js.map