...
describe('releaseChannels', () => {
    ...
    describe('invalid market and engine types', () => {
        ...
        it('throws an error if the base engine does not exist for symbol', () => {
            ...
            return expect(releaseChannels({ params, logger, engines, orderbooks }, { ReleaseChannelsResponse })).to.eventually.be.rejectedWith(PublicError, `No engine available for BTC`);
        });
        it('throws an error if the counter engine does not exist for symbol', () => {
            engines = new Map([['BTC', baseEngineStub]]);
            return expect(releaseChannels({ params, logger, engines, orderbooks }, { ReleaseChannelsResponse })).to.be.rejectedWith(PublicError, `No engine available for LTC`);
        });
    });
...
//# sourceMappingURL=release-channels.spec.js.map