...
const { RPC_ADDRESS, DATA_DIR, RELAYER_RPC_HOST, RELAYER_CERT_PATH, MARKETS, INTERCHAIN_ROUTER_ADDRESS, ID_PRIV_KEY, ID_PUB_KEY, DISABLE_AUTH, DISABLE_RELAYER_AUTH, RPC_PRIV_KEY, RPC_PUB_KEY, RPC_USER, RPC_PASS } = process.env;
program
    .version(CLI_VERSION)
...
program
    .action((args, opts) => {
    const { rpcAddress, dataDir, markets, interchainRouterAddress, relayerHost: relayerRpcHost, relayerCertPath, idPrivkeyPath: privIdKeyPath, idPubkeyPath: pubIdKeyPath, disableAuth, rpcPrivkeyPath: privRpcKeyPath, rpcPubkeyPath: pubRpcKeyPath, rpcUser, rpcPass } = opts;
    const engines = {};
    for (let currency of currencies) {
        let lowerSymbol = currency.symbol.toLowerCase();
...
//# sourceMappingURL=sparkswapd.js.map