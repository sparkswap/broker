...
describe('Orderbook', () => {
    ...
    describe('getTrades', () => {
        ...
        it('does not add a lowerbound if no since date is provided', async () => {
            timestamp = undefined;
            await orderbook.getTrades(timestamp, limit);
            expect(getRecordsStub).to.have.been.calledWith(eventStore, bound, { limit });
        });
        it('calls get records with a timestamp', async () => {
            await orderbook.getTrades(timestamp, limit);
            expect(getRecordsStub).to.have.been.calledWith(eventStore, bound, { gte: '1537526431034000000', limit });
        });
    });
    describe('getOrderbookEventsByTimestamp', () => {
        ...
        it('calls get records with a timestamp', () => {
            expect(getRecordsStub).to.have.been.calledWith(orderbookStore, sinon.match.func, rangeFromTimestamp);
    ...
    describe('getMarketEventsByTimestamp', () => {
        ...
        it('calls get records with a timestamp', () => {
            expect(getRecordsStub).to.have.been.calledWith(eventStore, sinon.match.func, rangeFromTimestamp);
            expect(rangeFromTimestampStub).to.have.been.calledWith(timestamp);
        });
    });
});
